public class AlphaBetaLMRTime extends Joueur {
    private long time;

    AlphaBetaLMRTime (long time) {
        this.time=time;
    }
    ...
    public void jouer (Echecs echecs) {
        if (echecs.getVictoire()==0) {
            ...
            long duration;
            do {
                alphaBeta = new AlphaBetaLMR(profondeur,restant);
                Echecs echecs2 = echecs.clone();
                echecs2.activateRollBack();
                alphaBeta.setBegin(System.currentTimeMillis());
                resTemp = alphaBeta.simuler(echecs2, profondeur, 0,-999999,999999, resTemp);
